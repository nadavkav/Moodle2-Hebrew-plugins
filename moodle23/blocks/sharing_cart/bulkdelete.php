...
global $DB;
	...
	if (is_array($delete = optional_param('delete'))) {
		...
		$items = $DB->get_records_select('sharing_cart',
		                            'user = '.$USER->id.' AND '.
	...
	print_header_simple($title, '', build_navigation($navlinks));
	{
		...
		if ($items = get_records('sharing_cart', 'user', $USER->id, 'tree,sort,text')) {
			echo '
			...
			foreach ($items as $id => $item) {
				...
				<li style="clear:left;">
					...
					<div style="float:left;">', sharing_cart_lib::get_icon($item->name, $item->icon), '</div>
					<div style="float:left;">
						<label for="delete_'.$id.'">', htmlspecialchars($item->text), '</label>
					</div>
				</li>';
				if (++$i % 10 == 0) {
					echo '
