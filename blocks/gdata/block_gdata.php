...
class block_gdata extends block_list {
    ...
    function get_content() {
        ...
        $title = get_string('syncnow', 'block_gdata');
        $icon = '<img src="'.$OUTPUT->pix_url('i/users') . '" class="icon" alt="' . $title . '" />';
        $this->content->items[] = "<a title=\"$title\" href=\"$CFG->wwwroot/blocks/gdata/index.php?hook=syncnow\">$icon&nbsp;$title</a>";
 ...
 **/
    ...
    function print_header() {
        ...
        if (in_array($this->hook, array('status', 'users', 'addusers'))) {
			...
			$row[]  = new tabobject('syncnow', $CFG->wwwroot.'/blocks/gdata/index.php?hook=syncnow', get_string('syncnow', 'block_gdata'));
            $tabs[] = $row;
            ...
            foreach ($users as $user) {
                ...
                switch ($hook) {
                    case 'users':
						...
						$syncnow = ' <a target="_new" href="rest.php?userid='.$user->id.'">Sync Now</a> ';
                        $table->add_data(array($username, fullname($user), $user->email, $lastsync, $syncnow . get_string("status$user->status", 'block_gdata')));
                        break;
