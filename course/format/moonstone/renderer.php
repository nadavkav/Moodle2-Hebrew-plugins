...
class format_moonstone_renderer extends format_section_renderer_base {
    ...
    protected function start_section_list() {
        return html_writer::start_tag('ul', array('class' => 'moonstone', 'style' => 'width:'.$this->tccolumnwidth.'%; float:left; padding:'.$this->tccolumnpadding.'px;'));
    }
    ...
    protected function section_header($section, $course, $onsectionpage) {
        ...
        $sectionstyle = '';
        global $tcsetting;
        $toggletext = get_string('moonstonetoggle', 'format_moonstone'); // The word 'Toggle'.
        ...
        if ($onsectionpage == false) {
            ...
            if (((!$onsectionpage) || ($tcscreenreader == false)) && ($section->section != 0)) {
                ...
                $o.= ( empty($section->summary) && has_capability('moodle/course:update', $context) ) ? get_string('pleaseaddsummary','format_moonstone') : $this->format_summary_text($section);
            ...
            $o.= $this->output->heading($this->section_title($section, $course), 3, 'section0');
        ...
        if ($PAGE->user_is_editing() and has_capability('moodle/course:update', $context)) {
            ...
            $url = new moodle_url('/course/changenumsections.php',
                            array('courseid' => $course->id,
                                'increase' => true,
                                'sesskey' => sesskey()));
            $icon = $this->output->pix_icon('t/addtopic', $straddsection);
            ...
            if ($course->numsections > 0) {
                ...
                $url = new moodle_url('/course/changenumsections.php',
                                array('courseid' => $course->id,
                                    'increase' => false,
                                    'sesskey' => sesskey()));
                $icon = $this->output->pix_icon('t/removetopic', $strremovesection);
    ...
    public function settings($course) {
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/course:update', $coursecontext)) {
            ...
            $o.= html_writer::start_tag('div', array('class' => 'content'));
            $o.= html_writer::start_tag('div', array('class' => 'sectionbody'));
            $o.= html_writer::tag('a', html_writer::tag('div', get_string('formatsettings','format_moonstone'), array('id' => 'set-settings')), array('title' => get_string("settings"), 'href' => 'format/moonstone/forms/settings.php?id=' . $course->id . '&sesskey=' . sesskey()));
            $o.= html_writer::end_tag('div');
            $o.= html_writer::end_tag('div');
            $o.= html_writer::end_tag('li');
        }
    ...
    public function toggle_all() {
        ...
        if ($tcscreenreader == false) { // No need to show if in screen reader mode.
            ...
            $o.= html_writer::tag('a', get_string('moonstoneopened', 'format_moonstone'), array('class' => 'on', 'href' => '#', 'onclick' => 'all_opened(); return false;'));
            $o.= html_writer::tag('a', get_string('moonstoneclosed', 'format_moonstone'), array('class' => 'off', 'href' => '#', 'onclick' => 'all_closed(); return false;'));
            $o.= html_writer::tag('span', get_string('moonstoneall', 'format_moonstone'), null);
