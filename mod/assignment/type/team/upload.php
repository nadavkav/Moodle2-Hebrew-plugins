...
$instance = new assignment_team($cm->id, $assignment, $cm, $course);
$team = $instance->get_user_team($formdata->userid);
...
$filemanager_options = array('subdirs'=>1, 'maxbytes'=>$assignment->maxbytes, 'maxfiles'=>$assignment->var1, 'accepted_types'=>'*', 'return_types'=>FILE_INTERNAL);
    ...
    $mform = new mod_assignment_team_upload_form(null, array('contextid'=>$contextid, 'userid'=>$formdata->userid, 'options'=>$filemanager_options, 'teamid'=>$team->id));
...
if ($instance->can_upload_file($submission,$team->id) && ($id==null)) {
	$data = new stdClass();
    // move submission files to user draft area
    $data = file_prepare_standard_filemanager($data, 'files', $filemanager_options, $context, 'mod_assignment', 'team_submission', $team->id);
